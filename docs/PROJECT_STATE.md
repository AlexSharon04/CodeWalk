...
## Recent decisions
- **2026-05-06** — **Walkthrough state persistence across reload + tab close.** Both `SegmentStore` and `WalkSession` now accept an optional `vscode.Memento` (wired to `context.workspaceState` in `extension.ts`). Every state-mutating call writes through fire-and-forget; an explicit `rehydrate()` method (called *after* all listeners are wired in `activate()`) restores state and fires `onDidChange` so the highlighter, lens provider, sidebar, and status bar all rebuild their views. End Walkthrough wipes both keyspaces (`segments:*` and `walkSession:state`). The `onDidCloseTextDocument → store.clear(uri)` listener was removed — closing a tab no longer evicts that file's segments, since the persistence intent is "everything stays until the user runs End Walkthrough." Behavior change worth noting: ad-hoc `Start CodeWalk` on a single file now leaves segments behind in `workspaceState` until End is invoked. `workspaceState` (not `globalState`) so segments can't leak across repos. Line-by-line decorations are intentionally NOT persisted across reload — they're render state, not logical state; the user re-toggles the button on return. Tests cover persistence keys, rehydrate fan-out, malformed-entry eviction, out-of-bounds clamping, and the no-memento path (production-safe degrade). Phase 4 line-by-line cache (in `globalState`) was already persistent and is unaffected.
...
- **Persistence verification quick-check.** With reload-persistence in place, run `Start CodeWalk` on a file → click a CodeLens → reload window → expect the CodeLenses + decorations to come back automatically and Alt+↓ to navigate to the next block immediately. Run `End Walkthrough` → reload → expect a clean editor with no CodeLenses.
